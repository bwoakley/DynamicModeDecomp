...
C If we aren't the highest process, then wait for data
     ...
     &               ,MPI_COMM_WORLD,status,ierror)
        ...
        END DO
      ELSE
C If we are the highest process, solve from the top row
        DO J=NY+1,1,-1
        DO I=0,NX-1  
          G(I,J)=G(I,J)/B(I,J)
        END DO
     ...
     &               ,MPI_COMM_WORLD,ierror)
      ELSE
! Here, we are the lowest process, RANK=0, solve down to BCS
        J=0
        DO I=0,NX-1
          G(I,J)=(G(I,J)-C(I,J)*G(I,J+1))/B(I,J)
        END DO
...
! x-y slabs.
      ...
      do k=0,NZ-1
        do j=1,NY
          do i=0,NX-1
            write(15+RANK,*) 'i,j,k: ',i,j,k,A(i,k,j)
          end do
        end do
      end do

      write(*,*) 'A(2,2,1): ',A(2,2,1)
      ...
      do k=0,NZ-1
        do j=1,NY
          do i=0,NX-1
            ...
            write(20+RANK,*),'i,j,k: ',i,j,k,buffer(i,j,k)
          end do
        end do
      end do

         write(*,*) 'transfer size: ',(NX+2)*(NY+2)*(NZ+2)/NPROCS
...
! (The new wall locations are 1 and NY-(NPROCS-1) )
          ...
          do j=1,NY_t
          do i=0,NX_t-1
            write(30+RANK,*),'i,j,k: ',i,j,k,A(i,k,j)
          end do
          end do
          end do
