...
function [closedOrbits,orbits,varargout] = poincare_closed_orbit_multi(flow,shearline,PSList,varargin)
nargoutchk(1,3)
...
addParamValue(p,'periodicBc',[true,false],@islogical)
...
for i = 1:nPoincareSection
    ...
    if showGraph
        [closedOrbitsPos,orbitsPos,hFigure(i,1)] = poincare_closed_orbit(flow.domain,double(flow.resolution),shearline.etaPos,poincareSection,'odeSolverOptions',odeSolverOptions,'dThresh',dThresh,'nBisection',nBisection,'periodicBc',periodicBc,'showGraph',showGraph);
    else
        [closedOrbitsPos,orbitsPos] = poincare_closed_orbit(flow.domain,double(flow.resolution),shearline.etaPos,poincareSection,'odeSolverOptions',odeSolverOptions,'dThresh',dThresh,'nBisection',nBisection,'periodicBc',periodicBc,'showGraph',showGraph);
    end
    closedOrbits{i}{1} = closedOrbitsPos;
    orbits{i}{1} = orbitsPos;
    
    if showGraph
        [closedOrbitsNeg,orbitsNeg,hFigure(i,2)] = poincare_closed_orbit(flow.domain,double(flow.resolution),shearline.etaNeg,poincareSection,'odeSolverOptions',odeSolverOptions,'dThresh',dThresh,'nBisection',nBisection,'periodicBc',periodicBc,'showGraph',showGraph);
    else
        [closedOrbitsNeg,orbitsNeg] = poincare_closed_orbit(flow.domain,double(flow.resolution),shearline.etaNeg,poincareSection,'odeSolverOptions',odeSolverOptions,'dThresh',dThresh,'nBisection',nBisection,'periodicBc',periodicBc,'showGraph',showGraph);
    end
