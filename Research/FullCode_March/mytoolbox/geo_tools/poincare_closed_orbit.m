...
addParamValue(p,'periodicBc',[true,false],@(input)validateattributes(input,{'logical'},{'size',[1,2]}));
    ...
    for i = 1:nZeroCrossing
        ...
        if any([isempty(indx1),isempty(indx2)]) || indx2 <= indx1 || abs(indx1-indx2) ~=1
            ...
            for j = 1:nBisection
                ...
                if(length==0)
                    intp3=poincareSection.integrationLength;
                else
                    intp3=[0,1.1*length];
                end
                % bisect
                ...
                [p3finalPos,iEvent3] = integrate_line(intp3,p3,domain,resolution,periodicBc,vectorField,odeSolverOptions,poincareSection.endPosition,'checkDiscontinuity',checkDiscontinuity);
                if iEvent3 ~= 1
                    warning([mfilename,':bisectionOpenOrbit'],'open orbit in bisection method')
                end
