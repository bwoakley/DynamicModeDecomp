...
function position = integrate_vector(timespan,initialCondition,domain,flowResolution,flowPeriodicBc,vectorGrid)
odeSolverOptions = odeset;
narginchk(5,6)

%tmp = initialize_ic_grid(flowResolution,domain);
xVector = linspace(domain(1,1),domain(1,2),flowResolution(1));
yVector = linspace(domain(2,1),domain(2,2),flowResolution(2));
[positionX,positionY] = meshgrid(xVector,yVector);
% tmp1(:,1) = positionX(:);
% tmp1(:,2) = positionY(:);
% tmp = reshape(tmp1(:,1),fliplr(flowResolution));
 positionX = positionX(1,:);

%tmp = initialize_ic_grid(flowResolution,domain);
%tmp = reshape(tmp1(:,2),fliplr(flowResolution));
...
function status = ode_output(~,position,flag,vector,vectorInterpolant,domain,flowResolution,flowPeriodicBc,vectorGrid)
    ...
    if(length(find(isnan(position)))>0)
        ;
    else
...
% the last argument should be flowPeriodicBc, but the shortest distance
% should be determined before periodicity?
distance = drectangle(position,domain(1,1),domain(1,2),domain(2,1),domain(2,2),[false,false]);
...
%Do not check this. For the doubly periodic flow distanceRectangle=inf.
%distanceRectangle = drectangle(position,domain(1,1),domain(1,2),domain(2,1),domain(2,2),flowPeriodicBc);
distanceRectangle = 1000.*distancePoincare;
